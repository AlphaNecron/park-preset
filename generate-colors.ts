 ...
 */
export const createSurfaceVariant = (name: string, shade: string, opaque?: boolean) => {
  const pf = `${opaque ? '' : 'a'}`;
  return {
    bg: {
      DEFAULT: { value: { _light: `{colors.${name}.${pf}2}`, _dark: `{colors.${name}.${pf}2}` } },
      ...(opaque && {
        hover: { value: { _light: `{colors.${name}.${pf}3}`, _dark: `{colors.${name}.${pf}3}` } },
      }),
      active: { value: { _light: `{colors.${name}.${pf}4}`, _dark: `{colors.${name}.${pf}4}` } },
    },
    border: {
      DEFAULT: { value: { _light: `{colors.${name}.${pf}6}`, _dark: `{colors.${name}.${pf}6}` } },
      hover: { value: { _light: `{colors.${name}.${pf}7}`, _dark: `{colors.${name}.${pf}7}` } },
    },
    fg: {
